...
development_testing = True if repository is None else False


def get_google_fonts(google_api_token: str) -> dict:
    """Fetch font data from Google Fonts API"""

    url = f"https://www.googleapis.com/webfonts/v1/webfonts?key={google_api_token}"
...
def get_local_fonts(file_path: str) -> dict:
...
def generate_comparison_table(old_data: json, new_data: json) -> str:
    ...
    if "items" not in old_data or "items" not in new_data:
        return "🗿 No changes detected in font data"
...
def write_to_local_file(
    data: json, file_path: str, dev: bool = development_testing
) -> None:
    """Writes data to a local JSON file
    
    If dev is True, the function will not write to the file but will print a success message"""
...
if not google_api_token:
    summary = generate_summary(
        repository,
        job_name,
        error="This script requires Google Fonts API, please set the GOOGLE_API_TOKEN env variable",
    )
...
try:
    new_fonts_data = get_google_fonts(google_api_token)
...
if local_fonts_data and new_fonts_data:
    ...
    try:
        write_to_local_file(new_fonts_data, local_file_path)
    except Exception as e:
        summary = generate_summary(
